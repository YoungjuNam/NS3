...
NS_OBJECT_ENSURE_REGISTERED (AIntersectionPosition);

TypeId
AIntersectionPosition::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::AIntersectionPosition")
    .SetParent<PositionAllocator> ()
    .SetGroupName ("Mobility")
    .AddConstructor<AIntersectionPosition> ()
    .AddAttribute ("Num", "The number of nodes.",
                   UintegerValue (10),
                   MakeUintegerAccessor (&AIntersectionPosition::m_num),
                   MakeUintegerChecker<uint32_t> ())
    .AddAttribute ("Bound", "Total Space(m).",
                   DoubleValue (1.0),
                   MakeDoubleAccessor (&AIntersectionPosition::m_bound),
                   MakeDoubleChecker<double> ())
   .AddAttribute ("Grid", "The number of line.",
                  UintegerValue (10),
                  MakeUintegerAccessor (&AIntersectionPosition::m_grid),
                  MakeUintegerChecker<uint32_t> ())
  ;
  return tid;
}
AIntersectionPosition::AIntersectionPosition ()
  : m_current (0)
{
}

void
AIntersectionPosition::SetBound (double bound)
{
  m_bound = bound;
}
void
AIntersectionPosition::SetGrid (uint32_t grid)
{
  m_grid = grid;
}
void
AIntersectionPosition::SetN (uint32_t num)
{
  m_num = num;
}

double
AIntersectionPosition::GetBound (void) const
{
  return m_bound;
}
uint32_t
AIntersectionPosition::GetGrid (void) const
{
  return m_grid;
}
uint32_t
AIntersectionPosition::GetN (void) const
{
  return m_num;
}
Vector
AIntersectionPosition::GetNext (void) const
{
  double x = 0.0, y = 0.0;
  double width = m_grid * 3;

  int r = UniformRandomVariable ().GetValue (0, (m_bound-width)/(m_num/(2*m_grid)));

  if(m_current <= m_num/2){
    x = (m_bound-width)/2 + 1.5 + 3*(m_current/(m_num/(2*m_grid)));
    y = (m_current%(m_num/(2*m_grid))) * ((m_bound-width)/(m_num/(2*m_grid))) + r + (m_current%(m_num/(2*m_grid)))/((m_num/(2*m_grid))/2) * width;
  }else{
    int i = m_current - m_num/2;
    x = (i%(m_num/(2*m_grid))) * ((m_bound-width)/(m_num/(2*m_grid))) + r + (i%(m_num/(2*m_grid)))/((m_num/(2*m_grid))/2) * width;
    y = (m_bound-width)/2 + 1.5 + 3*(i/(m_num/(2*m_grid)));
  }
...
int64_t
AIntersectionPosition::AssignStreams (int64_t stream)
{
  return 0;
}

NS_OBJECT_ENSURE_REGISTERED (IntersectionsPosition);

TypeId
IntersectionsPosition::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::IntersectionsPosition")
    .SetParent<PositionAllocator> ()
    .SetGroupName ("Mobility")
    .AddConstructor<IntersectionsPosition> ()
    .AddAttribute ("Num", "The number of nodes.",
                   UintegerValue (100),
                   MakeUintegerAccessor (&IntersectionsPosition::m_num),
                   MakeUintegerChecker<uint32_t> ())
    .AddAttribute ("Distance", "Distance between intersection (m).",
                   DoubleValue (300.0),
                   MakeDoubleAccessor (&IntersectionsPosition::m_bound),
                   MakeDoubleChecker<double> ())
   .AddAttribute ("Intersection", "The number of intersection n x n.",
                  UintegerValue (1),
                  MakeUintegerAccessor (&IntersectionsPosition::m_intersection),
                  MakeUintegerChecker<uint32_t> ())
   .AddAttribute ("Grid", "The number of line.",
                  UintegerValue (2),
                  MakeUintegerAccessor (&IntersectionsPosition::m_grid),
                  MakeUintegerChecker<uint32_t> ())
  ;
  return tid;
}
IntersectionsPosition::IntersectionsPosition ()
  : m_current (0)
{
}

void
IntersectionsPosition::SetBound (double bound)
{
  m_bound = bound;
}
void
IntersectionsPosition::SetGrid (uint32_t grid)
{
  m_grid = grid;
}
void
IntersectionsPosition::SetN (uint32_t num)
{
  m_num = num;
}

double
IntersectionsPosition::GetBound (void) const
{
  return m_bound;
}
uint32_t
IntersectionsPosition::GetGrid (void) const
{
  return m_grid;
}
uint32_t
IntersectionsPosition::GetN (void) const
{
  return m_num;
}
Vector
IntersectionsPosition::GetNext (void) const
{
  double x = 0.0, y = 0.0;
  int a = UniformRandomVariable ().GetValue (0, 100 * m_intersection);
  int b = UniformRandomVariable ().GetValue (0, 100);
  int c = UniformRandomVariable ().GetValue (0, 100 * m_grid);
  int d = 0;
  if(b < 50) d = 1; else d = -1;

  if(m_current < m_num / 2){
    x = m_bound * (a/100 + 0.5) + d * (3 * (c / 100) + 1.5);
    y = UniformRandomVariable ().GetValue (0, m_intersection * m_bound);
  } else {
    x = UniformRandomVariable ().GetValue (0, m_intersection * m_bound);
    y = m_bound * (a/100 + 0.5) + d * (3 * (c / 100) + 1.5);
  }
...
int64_t
IntersectionsPosition::AssignStreams (int64_t stream)
{
  return 0;
}
} // namespace ns3 
