...
class AIntersectionPosition : public PositionAllocator
{
public:
  static TypeId GetTypeId (void);
  ...
  AIntersectionPosition ();

  void SetBound (double bound);
  void SetGrid (uint32_t grid);
  void SetN (uint32_t num);

  double GetBound (void) const;
  uint32_t GetGrid (void) const;
  uint32_t GetN (void) const;
...
private:
  mutable uint32_t m_current; //!< currently position
  uint32_t m_num;
  double m_bound; //!< minimum boundary on x positions
  uint32_t m_grid;  //!< number of positions to allocate on each row or column
};

class IntersectionsPosition : public PositionAllocator
{
public:
  static TypeId GetTypeId (void);

  IntersectionsPosition();
